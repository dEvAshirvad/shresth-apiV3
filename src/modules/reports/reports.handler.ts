...
export class KpiReportHandler {
  static async list(req: Request, res: Response) {
    ...
    return Respond(
      res,
      { reports, message: 'Reports fetched successfully' },
      200
    );
  }
  ...
  static async getSummary(req: Request, res: Response) {
    ...
    const run = await KpiReportService.getReportSummary(
      organizationId,
      periodId
    );
    ...
    return Respond(
      res,
      { run, message: 'Report summary fetched successfully' },
      200
    );
  }
  ...
  static async departmentRoleStats(req: Request, res: Response) {
    ...
    return Respond(
      res,
      { stats, message: 'Department role stats fetched' },
      200
    );
  }
  ...
  static async ranking(req: Request, res: Response) {
    ...
    if (scope !== 'overall' && scope !== 'department') {
      throw new APIError({
        STATUS: 400,
        TITLE: 'INVALID_SCOPE',
        MESSAGE: 'scope must be overall|department',
      });
    }
    if (scope === 'department' && !departmentId) {
      throw new APIError({
        STATUS: 400,
        TITLE: 'MISSING_DEPARTMENT',
  ...
  static async listWhatsAppSends(req: Request, res: Response) {
    ...
    return Respond(
      res,
      { ...result, message: 'WhatsApp send records fetched' },
      200
    );
  }
  ...
  static async sendWhatsAppPerformance(req: Request, res: Response) {
    ...
    if (env.BACKGROUND_JOBS_SYNC) {
      const data =
        await WhatsappPerformanceService.sendPerformanceBatch(payload);
  ...
  static async downloadDepartmentZip(req: Request, res: Response) {
    ...
    const fileName = String(
      (artifact as any).fileName || 'department-reports.zip'
    );
    if (!filePath) {
      throw new APIError({
        STATUS: 404,
